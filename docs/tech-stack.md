...
| Fonts | `@fontsource/ibm-plex-mono`, `@fontsource/ibm-plex-sans`, `@fontsource/newsreader`, `@fontsource/space-grotesk`, `@fontsource/spline-sans-mono` | 5.x | Self-hosted (offline + privacy + AGPL ethos). Back the per-user `fontStyle` themes (Plex Mono / Editorial / Grotesk / Typewriter); Plex Mono+Sans is the default pair. Supersedes Inter+Caveat — see D19. |

...
### D19 — Fonts: self-hosted IBM Plex + a 4-theme `fontStyle` selector (supersedes Inter+Caveat) ✅ 🔒-ish
**Why:** the `/design` `Openweek v2 (paper)` export replaces the v1 Inter (body) + Caveat (accent) pairing with a
**monospaced display face** and offers a font-theme selector. The export loads from the Google Fonts CDN, which
conflicts with the offline/privacy/self-host goal. **Decision:** keep the selector but **self-host via
`@fontsource/*`** (no CDN). `fontStyle` is a per-user setting with four pairings driven by `--ow-display` /
`--ow-body`: **Plex Mono** (IBM Plex Mono + Plex Sans, default), **Editorial** (Newsreader + Plex Sans),
**Grotesk** (Space Grotesk), **Typewriter** (Spline Sans Mono + Plex Sans). The handwritten Caveat accent is
dropped. Default **accent is sky** (`#CBDDE9`). Reinforces D9/tech-stack's "no Google Fonts CDN" stance. _User
chose self-host + keep the selector._
